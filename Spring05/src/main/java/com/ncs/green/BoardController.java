...
// 출력 PageNo 제한하기
   ...
   public ModelAndView plist2(ModelAndView mv, PageVO pvo) {
      ...
      // 4. 출력할 만큼만 record select
      // => sno, eno
      int sno = ((currPage - 1)* pvo.getPerPage()) +1;
      int eno = sno+ pvo.getPerPage()-1;   // 21~
      pvo.setSno(sno);
      pvo.setEno(eno);
      
      ArrayList<BoardVO> blist = service.pageList(pvo);
      
      // 5. View 에서 필요한 자료만 addObject
