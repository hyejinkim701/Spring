...
public class MemberDAO {
		...
		try {
			...
			if (rs.next()) {
				do {
					...
					vo.setWeight(rs.getDouble(6));
					vo.setBirthd(rs.getString(7));
					vo.setUploadfile(rs.getString(8));
					mList.add(vo);
				}while (rs.next());
			}else {
	...
	public int mUpdate(MemberVO vo) {
	      sql="update member set password=?,name=?,lev=?,"
	         +"pay=?,weight=?,birthd=?,uploadfile=? where id=?"   ;
	      int cnt=0 ;
	      try {
	         pst=cn.prepareStatement(sql);
	         ...
	         pst.setDouble(5,vo.getWeight());
	         pst.setString(6,vo.getBirthd());
	         pst.setString(7, vo.getUploadfile());
